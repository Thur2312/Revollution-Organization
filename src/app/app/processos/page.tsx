...
import { SquaresFour, Stamp } from '@phosphor-icons/react/dist/ssr'
import { supabase } from '../../../lib/supabaseClient'
import { useAppSession } from '../../../lib/AppSessionContext'

type Workspace = { id: string; name: string }

export default function ProcessosLandingPage() {
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      if (!mounted) return
      // Single workspace: skip the picker and go straight to its Processos.
      // Multiple workspaces: let the user choose which one here.
      if (list.length === 1) router.replace(`/app/workspace/${list[0].id}/processos`)
  ...
  if (hasNoWorkspace) {
    return (
      <div className="mx-auto max-w-6xl px-6 py-10">
        ...
        <p className="mb-8 text-sm text-muted-foreground">Acompanhamento de processos do INPI.</p>
        ...
        <div className="flex flex-col items-center gap-2 rounded-xl border border-dashed border-border py-14 text-center">
          ...
          <p className="text-sm text-muted-foreground">Crie um workspace primeiro para acompanhar processos.</p>
          <Link
            href="/app"
  ...
  if (workspaces) {
    return (
      <div className="mx-auto max-w-6xl px-6 py-10">
        <h1 className="mb-1 text-2xl font-semibold tracking-tight text-primary">Processos</h1>
        <p className="mb-8 text-sm text-muted-foreground">Escolha o workspace para ver os processos dele.</p>

        <ul className="divide-y divide-border rounded-xl border border-border bg-background">
          {workspaces.map((w) => (
            <li key={w.id}>
              <Link
                href={`/app/workspace/${w.id}/processos`}
              ...
              >
                ...
                <span className="hidden shrink-0 text-xs text-muted-foreground sm:inline">Ver processos →</span>
              </Link>
            </li>
          ))}
        </ul>
